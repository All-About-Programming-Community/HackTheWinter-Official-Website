...
const Navbar = () => {
    ...
    return (
        <div className="w-full h-fit text-black flex sm:flex-row xs:flex-col xs:justify-around sm:justify-start gap-12 sm:items-center bg-transparent sticky top-0 z-[100] bg-gradient-to-r from-white via-transparent">
            ...
            <div className={active ? "active-nav absolute pt-28 h-screen flex gap-12 xs:flex-col items-center" : "nav flex sm:flex-row sm:gap-12 xs:flex-col xs:items-center"}>
                {menu.map((item, index) => {
                    return (
                        <div key={index} className="text-xl nav-item box-border  flex items-center xs:font-semibold">

                            <Link href={item.route} className="nav-link" onClick={() => {if(window.innerWidth < 640){setActive(!active)}}}>
                                {item.label}
                            </Link>
                        </div>
                    )
